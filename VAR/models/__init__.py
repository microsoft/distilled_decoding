...
def build_vae_dd(
    ...
    init_adaln=0.5, init_adaln_gamma=1e-5, init_head=0.02, init_std=-1,    # init_std < 0: automated
    # DD args
    n_d_embed_enable=True, layerwise_n_d_embed=True,
) -> DD_Model:
    heads = depth
    width = depth * 64
    dpr = 0.1 * depth/24
    ...
    dd = DD_Model(
        ...
        patch_nums=patch_nums,
        flash_if_available=flash_if_available, fused_if_available=fused_if_available,
        n_d_embed_enable=n_d_embed_enable, layerwise_n_d_embed=layerwise_n_d_embed,
    ).to(device)
    dd.init_weights(init_adaln=init_adaln, init_adaln_gamma=init_adaln_gamma, init_head=init_head, init_std=init_std)
