...
def intermediate_prefill(model, cond_idx: torch.Tensor, prefix: torch.Tensor, input_pos: torch.Tensor, cfg_scale: float, **sampling_kwargs):
    if cfg_scale > 1.0:
        prefix_combined = torch.cat([prefix, prefix])
        logits, _ = model(prefix_combined, cond_idx, input_pos)
        logits_combined = logits
        cond_logits, uncond_logits = torch.split(logits_combined, len(logits_combined) // 2, dim=0)
        logits = uncond_logits + (cond_logits - uncond_logits) * cfg_scale
    else:
...
@torch.no_grad()
def intermediate_generate(model, cond, prefix, t_start, max_new_tokens, emb_masks=None, cfg_scale=1.0, cfg_interval=-1, **sampling_kwargs):
    if model.model_type == 'c2i':
        if cfg_scale > 1.0:
            cond_null = torch.ones_like(cond) * model.num_classes
            cond_combined = torch.cat([cond, cond_null])
    ...
    # create an empty tensor of the expected final shape and fill in the current tokens
    seq = torch.empty((max_batch_size, T_new+t_start), dtype=torch.int, device=device)
    seq[:, T:T+t_start] = prefix
    input_pos = torch.arange(0, T+t_start, device=device)
    next_token = intermediate_prefill(model, cond_combined, prefix, input_pos, cfg_scale, **sampling_kwargs)
    seq[:, T+t_start:T+t_start+1] = next_token

    input_pos = torch.tensor([T+t_start], device=device, dtype=torch.int)
    generated_tokens, _ = decode_n_tokens(model, next_token, input_pos, max_new_tokens-1, cfg_scale, cfg_interval, **sampling_kwargs)
    seq[:, T+t_start+1:] = torch.cat(generated_tokens, dim=1)
